...
const distritosLima = [
  "Ate", "Barranco", "Breña", "Carabayllo", "Chaclacayo", "Chorrillos", "Cieneguilla", "Comas", "El Agustino",
  "Independencia", "Jesús María", "La Molina", "La Victoria", "Lima", "Lince", "Los Olivos", "Lurigancho",
  "Lurin", "Magdalena del Mar", "Miraflores", "Pachacamac", "Pucusana", "Puente Piedra", "Pueblo Libre",
...
const RegisterPage = () => {
  ...
  const handleChange = (e) => {
    if (["nombre", "apellidos", "contactoEmergenciaNombre"].includes(e.target.name)) {
      ...
      if (!regexLetras.test(e.target.value)) return;
    }

    if (e.target.name === "contactoEmergenciaWhatsApp") {
      const regexNumeros = /^[0-9\b]*$/;
      if (!regexNumeros.test(e.target.value)) return;
    }
  ...
  return (
    <div className="register-container">
      ...
      <form onSubmit={handleSubmit} className="register-form" noValidate>
        ...
        <label>
          Edad
          <input type="number" name="edad" value={formData.edad} onChange={handleChange} placeholder="Edad" required min={10} max={120} />
        ...
        <fieldset style={{ border: "1px solid #ccc", padding: "10px", marginBottom: "15px" }}>
          ...
          <label>
            Nombre
            <input type="text" name="contactoEmergenciaNombre" value={formData.contactoEmergenciaNombre} onChange={handleChange} placeholder="Nombre" required />
            {errors.contactoEmergenciaNombre && <p className="error">{errors.contactoEmergenciaNombre}</p>}
          </label>

          <label>
            Correo (gmail)
            <input type="email" name="contactoEmergenciaCorreo" value={formData.contactoEmergenciaCorreo} onChange={handleChange} placeholder="correo@gmail.com" required />
            {errors.contactoEmergenciaCorreo && <p className="error">{errors.contactoEmergenciaCorreo}</p>}
          </label>

          <label>
            Número WhatsApp
            <input type="tel" name="contactoEmergenciaWhatsApp" value={formData.contactoEmergenciaWhatsApp} onChange={handleChange} placeholder="9 dígitos, empieza con 9" required maxLength={9} pattern="[9]{1}[0-9]{8}" title="Número de WhatsApp válido (9 dígitos, empieza con 9)" />
